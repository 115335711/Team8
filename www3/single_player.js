(function() {
	...
	function init() {
		/**
		*Gets the canvas to draw on and calls all the functions necessary to play the game.
		*/
	...
	function main() {
		...
		*/
		...
		if (started == false && gotDeck == false && madeBet == false) {
			...
			//ask the layer how many decks to use
			...
			//ask the player how much he/she wants to bet
			...
			//stat the game proper
			window.clearInterval(mainID); //once the game is set up, main() doesn't need to be constantly called
			tooMuch = false;
			deck = [];
	...
	function eyes() {
		/**
		*Keeps the player updated regarding his/her various "statuses" and options.
		*/
		if (madeBet == true) {
			//display the player's "statuses"
			...
			if (done == false) {
				...
				//game over, let the player what the outcome is
	...
	function clear(x, y, w, h) {
		/**
		*Sets up the canvas by clearing it and re-colouring it.
		*/
	...
	function play() {
		...
		*/
		...
		context.stroke();
		//give the player and the dealer 2 cards
	...
	function getDealerToPlay() {
		...
		*/
		...
		if (dealer.hand[0].card == "Ace") {
			//if the dealer has an Ace as its first card...
			if (player.insured == true) {
				//...and the player is insured...
				...
				if (dealer.hand[1].point == 10){
					...
					//...but the dealer doesn't have a blackjack
				}
				player.sideBet = 0;
			}
		}
		if ((dealer.hand[0].card == "Ace" && dealer.hand[1].point == 10) || (dealer.hand[1].card == "Ace" && dealer.hand[0].point == 10) && done == false) {
			...
			//if the player has a blackjack but dealer doesn't
			player.money += ((player.mainBet*2) + (player.mainBet/2));
			player.mainBet = 0;
			done = true;
		}
		else {
			//if neither party has a blackjack
			var i = 1;
			while (done == false && 17 > dealer.points[dealer.points.length - 1]) {
				//while the dealer doesn't have at least a soft 17, keep drawing cards and display them
				i++;
				dealer.hand.push(deck.pop());
			...
			if ((dealer.points.length == 0 && player.points.length > 0) || Math.max.apply(null, player.points) > Math.max.apply(null, dealer.points)) {
				...
				//if the player has less points
				ending = "losing";
			}
			else {
				//if both parties have the same points
				ending = "push";
				player.money += player.mainBet;
			}
			player.mainBet = 0;
	...
	function log() {
		/**
		*Logs the ending string in the console tab of the browser for testing purposes.
		*/
		console.log(ending);
	}
	...
	function restart() {
		...
		*/
		window.clearInterval(eyesID); //once the game is over, eyes() doesn't need to be constantly called
		clear(0, 0, width, height);
		log();
	...
	function quit() {	
		/**
		*Quits the user out of the game and takes him/her to the Index page.
		*/
		window.removeEventListener('keydown', activate, false);
		window.clearInterval(eyesID); //once the game is over, eyes() doesn't need to be constantly called
